...
def custom_objective_quat(num_coords,interval_value):
    ...
    AC_pos = Qrm(x[0:4]) * position([0.6,0,0])
    ...
    obj_scapula_thorax = sp.Matrix([interval_value*sum((scapula_thorax-sp.Matrix(x_traj[4:8])).applyfunc(lambda x: x**2))])
    # obj_GH_rot = sp.Matrix([interval_value*sum((GH_rot-sp.Matrix(x_traj[8:12])).applyfunc(lambda x: x**2))])
    obj_humerus_thorax = sp.Matrix([interval_value*sum((humerus_thorax-sp.Matrix(x_traj[8:12])).applyfunc(lambda x: x**2))])
...
def custom_objective_eul(num_coords,interval_value):
    ...
    humerus_thorax = sp.Matrix([humerus_thorax_x,humerus_thorax_y,humerus_thorax_z])
    #YZY
...
def polynomials_euler(OS_model,derive,model_params_struct, gen_matlab_functions = None):
    ...
    qpol = q_new
    nmus = len(OS_model['model']['muscles'][0,0][0])
    ...
    if derive == 'symbolic':
        ...
        for i in range(nmus):
            ...
            muscle = OS_model['model']['muscles'].item()[0,i]
    ...
    for imus in range(nmus):
        muscle = OS_model['model']['muscles'].item()[0,imus]
        ...
        if isWrapped == 0:
            ...
            # print('analytic')
            L = analytic_length_eul(origin, insertion, O_pos, I_pos, qpol[3:], OS_model)
    ...
    if derive == 'symbolic':
        conoid_length = sp.symbols('conoid_length')
        conoid_stiffness = sp.symbols('conoid_stiffness')
    ...
    if gen_matlab_functions == 1:
        ...
        q_subs_dict = dict(zip(qpol[3:-1],qsubs))
        segments = []
...
def polynomials_quat(OS_model,derive,model_params_struct, gen_matlab_functions = None):
    ...
    for imus in range(nmus):
        ...
        if isWrapped == 0:
            ...
            O_pos = muscle['origin_position'].item()[0]
            I_pos = muscle['insertion_position'].item()[0]
            L = analytic_length_quat(origin, insertion, O_pos, I_pos, q_new[4:], OS_model)
    ...
    F_conoid = conoid_force(conoid_L, conoid_length, conoid_stiffness, conoid_eps)
    Jac_conoid = -sp.Matrix([conoid_L]).jacobian(q_new).T                
...
def create_eoms_quat(model_params_struct, derive = 'symbolic',gen_matlab_functions = None):
    ...
    if gen_matlab_functions == 1:
        ...
        other_constants = {'offset_humerus_rot':list(offset_humerus_rot),'EL_rot_axis': list(EL_rot_axis),'PSY_rot_axis': list(PSY_rot_axis),
                            ...
                            'k_contact_out': k_contact_out,'eps_out': eps_out,'first_elips_scale':first_elips_scale,
...
def create_eoms_eul(model_params_struct, derive = 'symbolic',gen_matlab_functions = None):
    # symbols
    t = sp.symbols('t')
    g,c = sp.symbols('g,c')  # 
    states = ['q','u']
    ...
    if gen_matlab_functions == 1:
        ...
        other_constants = {'offset_humerus_rot':list(offset_humerus_rot),'EL_rot_axis': list(EL_rot_axis),'PSY_rot_axis': list(PSY_rot_axis),
                            'k_contact_in': k_contact_in,'eps_in': eps_in,'contTS': list(contTS),
                            'contAI': list(contAI), 'elips_trans':list(elips_trans), 'elips_dim':list(elips_dim),'first_elips_scale':first_elips_scale,
...
def MatlabFunction(function,fun_name,assignto,coordinates,speeds,inputs,body_constants,segments,other_constants,muscle_constants,parameters,folder):
    list_of_variables = [speeds,inputs, [body_constants, other_constants], muscle_constants, parameters]
    ...
    with open(f"{folder}/{fun_name}.m","w") as text_file:
        ...
        for var_name, var_tuple in parameters.items():
            if not type(var_tuple) == list:
                print(f'{var_name} = model.{var_name};',file=text_file)
            else:
                for i, elem in enumerate(var_tuple):
...
def create_eoms_u0state(model_params_struct, derive = 'symbolic',gen_matlab_functions = None):
    # symbols
    t = sp.symbols('t')
